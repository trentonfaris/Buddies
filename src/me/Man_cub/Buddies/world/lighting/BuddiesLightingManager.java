...
public abstract class BuddiesLightingManager extends LightingManager<BuddiesCuboidLightBuffer> {
	...
	@Override
	public BuddiesCuboidLightBuffer deserialize(Modifiable holder, int baseX, int baseY, int baseZ, int sizeX, int sizeY, int sizeZ, byte[] data) {
		return new BuddiesCuboidLightBuffer(holder, getId(), baseX, baseY, baseZ, sizeX, sizeY, sizeZ, data);
	}
	...
	@Override
	public BuddiesCuboidLightBuffer newLightBuffer(Modifiable holder, int baseX, int baseY, int baseZ, int sizeX, int sizeY, int sizeZ) {
	...
	public void processLower(Iterable<IntVector3> coords, IntVector4ExpandableFIFO fifo, IntVector4ExpandableFIFO regen, ChunkCuboidLightBufferWrapper<BuddiesCuboidLightBuffer> light, ImmutableCuboidBlockMaterialBuffer material, ImmutableHeightMapBuffer height) {
		Iterator<IntVector3> itr = coords.iterator();
	...
	public int getIncomingLight(ChunkCuboidLightBufferWrapper<BuddiesCuboidLightBuffer> light, ImmutableCuboidBlockMaterialBuffer material, int x, int y, int z, ByteBitSet occlusionSet, BlockFace face) {
	...
	public int getLightLevel(ChunkCuboidLightBufferWrapper<BuddiesCuboidLightBuffer> light, int x, int y, int z) {
		return getLightLevel(light, x, y, z, false);
	}

	public int getLightLevel(ChunkCuboidLightBufferWrapper<BuddiesCuboidLightBuffer> light, int x, int y, int z, boolean allowNull) {
		BuddiesCuboidLightBuffer buffer = light.getLightBuffer(x, y, z, allowNull);
		int level = 0;
		if (buffer != null) {
	...
	public void setLightLevel(ChunkCuboidLightBufferWrapper<BuddiesCuboidLightBuffer> light, int x, int y, int z, int level) {
		BuddiesCuboidLightBuffer buffer = light.getLightBuffer(x, y, z);
		if (buffer != null) {
	...
	public void purgeSet(ChunkCuboidLightBufferWrapper<BuddiesCuboidLightBuffer> light, TInt10TripleSet[] sets) {
		for (int i = 0; i < sets.length; i++) {
	...
	protected void checkAndAddDirtyFalling(TInt10TripleSet[] dirtySets, TInt10TripleSet[] regenSets, ChunkCuboidLightBufferWrapper<BuddiesCuboidLightBuffer> light, ImmutableCuboidBlockMaterialBuffer material, ImmutableHeightMapBuffer height, int x, int y, int z) {
	...
	protected void checkAndAddDirtyRising(TInt10TripleSet[] dirtySets, ChunkCuboidLightBufferWrapper<BuddiesCuboidLightBuffer> light, ImmutableCuboidBlockMaterialBuffer material, ImmutableHeightMapBuffer height, int x, int y, int z) {
		if (BlockMaterial.UNGENERATED.getId() == material.getId(x, y, z)) {
			return;
		}
		int actualLevel = getLightLevel(light, x, y, z);
	...
	protected void resolveLower(TInt10TripleSet[] dirtySets, TInt10TripleSet[] regenSets, ChunkCuboidLightBufferWrapper<BuddiesCuboidLightBuffer> light, ImmutableCuboidBlockMaterialBuffer material, ImmutableHeightMapBuffer height) {
	...
	@Override
	protected void initChunks(ChunkCuboidLightBufferWrapper<BuddiesCuboidLightBuffer> light, ImmutableCuboidBlockMaterialBuffer material, ImmutableHeightMapBuffer height, Chunk[] chunks) {
	...
	@Override
	public BuddiesCuboidLightBuffer[][][] bulkInitialize(ImmutableCuboidBlockMaterialBuffer buffer, int[][] height) {
		...
		BuddiesCuboidLightBuffer[][][] lightBufferArray = new BuddiesCuboidLightBuffer[cx][cy][cz];
		...
		for (int x = 0; x < cx; x++) {
			for (int y = 0; y < cy; y++) {
				for (int z = 0; z < cz; z++) {
					...
					BuddiesCuboidLightBuffer light = newLightBuffer(null, baseX + (cx << shift), baseY + (cy << shift), baseZ + (cz << shift), chunkSize, chunkSize, chunkSize);
