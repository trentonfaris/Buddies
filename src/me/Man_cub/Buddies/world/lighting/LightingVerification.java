...
public class LightingVerification {
	...
	private static boolean checkChunk(Chunk c, BuddiesLightingManager manager) {
		final BuddiesCuboidLightBuffer[][][] lightBuffers = getLightBuffers(c, manager);
		...
		LightGenerator lightSource = manager == BuddiesLighting.SKY_LIGHT ?
				new LightGenerator() {
					@Override
	...
	private static BuddiesCuboidLightBuffer[][][] getLightBuffers(Chunk c, BuddiesLightingManager manager) {
		BuddiesCuboidLightBuffer[][][] buffers = new BuddiesCuboidLightBuffer[3][3][3];
		for (int x = 0; x < 3; x++) {
			for (int y = 0; y < 3; y++) {
				for (int z = 0; z < 3; z++) {
	...
	private static BuddiesCuboidLightBuffer getLightBuffer(World w, int cx, int cy, int cz, short id) {
		Chunk c = w.getChunk(cx, cy, cz, LoadOption.LOAD_ONLY);
		if (c == null) {
			return null;
		}
		return (BuddiesCuboidLightBuffer) c.getLightBuffer(id);
	}
	...
	public static boolean testLight(int x, int y, int z, CuboidBlockMaterialBuffer[][][] materialBuffers, BuddiesCuboidLightBuffer[][][] lightBuffers, LightGenerator lightSource) {
	...
	private static int[][][] getNeighborLight(int x, int y, int z, BuddiesCuboidLightBuffer[][][] buffers) {
	...
	private static int getLight(int x, int y, int z, BuddiesCuboidLightBuffer[][][] buffers) {
		int xi = 1;
		int yi = 1;
		int zi = 1;
		if (x < 0) {
		...
		BuddiesCuboidLightBuffer b = buffers[xi][yi][zi];
		if (b == null) {
			return 0;
		}
	...
	private static void log(String message, int x, int y, int z, CuboidBlockMaterialBuffer[][][] material, BuddiesCuboidLightBuffer[][][] light, BlockMaterial[][][] localMaterials, int[][][] localLight) {
