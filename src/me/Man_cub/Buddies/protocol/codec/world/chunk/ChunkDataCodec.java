package me.Man_cub.Buddies.protocol.codec.world.chunk;
...
public final class ChunkDataCodec extends MessageCodec<ChunkDataMessage> {
	...
	public ChannelBuffer encode(ChunkDataMessage message) throws IOException {
		...
		if (session != null) {
			uncompressedData = message.getSession().getDataMap().get(BuddiesProtocol.CHUNK_NET_CACHE).handle(uncompressedData);
		}
